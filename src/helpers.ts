...
export default class Helpers {
    ...
    static tags: any = {
        ...
        route: { classes: ['Route'], functions: ['route', '$route'] },
        trans: {
            classes: ['Lang'],
            functions: ['__', 'trans', '@lang', 't', '$t'],
        },
        validation: {
            classes: ['Validator'],
    ...
    static runLaravel(code: string): Promise<string> {
        ...
        if (
            ...
            var command =
                ...
                "require_once '" +
                Helpers.projectPath('vendor/autoload.php', true) +
                "';" +
                "$app = require_once '" +
                Helpers.projectPath('bootstrap/app.php', true) +
                "';" +
            ...
            return new Promise(function (resolve, error) {
                self.runPhp(command)
                    .then(function (result: string) {
                        ...
                        out =
                            /___VSCODE_LARAVEL_EXTRA_INSTELLISENSE_OUTPUT___(.*)___VSCODE_LARAVEL_EXTRA_INSTELLISENSE_END_OUTPUT___/g.exec(
                                out,
                            );
                        if (out) {
                            resolve(out[1]);
                    ...
                    if (stdout.length > 0) {
                        ...
                        if (Helpers.outputChannel !== null) {
                            Helpers.outputChannel.appendLine(
                                'Laravel extra intellisense Error: ' + stderr,
                            );
                        }
                        error(stderr);
                    }
